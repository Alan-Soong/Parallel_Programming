...
void* butterfly_worker(void* args_ptr) {
    ButterflyArgs* args = (ButterflyArgs*)args_ptr;
    int *a = args->a, n = args->n, len = args->len, p = args->p;
    int *twiddles = args->twiddles;

    for (int blk = args->blk_start; blk < args->blk_end; ++blk) {
        int i = blk * len;
...
void butterfly_layer_simd(int *a, int n, int len, int wlen, int p, Montgomery &mont) {
    ...
    int num_threads = 4;
    pthread_t threads[4];
    ButterflyArgs args[4];

    int blocks = n / len;
    for (int t = 0; t < num_threads; ++t) {
        args[t] = {a, n, len, wlen, p, &mont,
                  t * blocks / num_threads,
                  (t + 1) * blocks / num_threads,
                  twiddles};
        pthread_create(&threads[t], nullptr, butterfly_worker, &args[t]);
    }
    for (int t = 0; t < num_threads; ++t) {
