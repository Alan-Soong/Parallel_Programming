...
#define NUM_THREADS 4  // 可根据机器核数配置
...
void poly_multiply(int *a, int *b, int *ab, int n, int p, MPI_Comm comm) {
    ...
    std::vector<int> local_C(end-start, 0);

    // ======= PTHREAD 调用 =========
    pthread_t threads[NUM_THREADS];
    ThreadData thread_data[NUM_THREADS];
    int work = end - start;
    int block = (work + NUM_THREADS - 1) / NUM_THREADS;

    BarrettReducer br(p);
    for (int i = 0; i < NUM_THREADS; ++i) {
        int t_start = start + i * block;
        int t_end = std::min(t_start + block, end);
        thread_data[i] = ThreadData{i, t_start, t_end, &A, &B, &local_C, start, &br};
