...
int main() {
    ...
    ofstream mat_vec_file("matrix_vector_results(1).csv");
    test_mat_vec(mat_vec, mat_vec_file);
    test_mat_vec(optimized_mat_vec, mat_vec_file);
    mat_vec_file.close();

    cout << "Program 2 results start saving......" << endl;

    ofstream sum_file("sum_results(1).csv");
    test_sum(normal_sum, sum_file);
    test_sum(optimized_sum, sum_file);
    test_sum(recursion_sum, sum_file);
    test_sum(sec_roll_sum, sum_file);
    sum_file.close();

    cout << "Benchmark results saved to files." << endl;
    return 0;
}
