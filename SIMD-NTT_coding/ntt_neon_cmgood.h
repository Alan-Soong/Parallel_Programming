...
// 单层NEON优化蝶形变换（预分配的twiddles传进来）
void butterfly_layer_simd(int *a, int n, int len, int p, const int* twiddles) {
...
void ntt_simd(int *a, int n, int w, int p) {
    ...
    // 预分配twiddles
    int *twiddles = (int*)aligned_alloc(64, (n / 2) * sizeof(int));
    if (!twiddles) {
    ...
    for (int len = 2; len <= n; len <<= 1) {
        ...
        for (int i = 1; i < len / 2; ++i) {
            twiddles[i] = (int)((int64_t)twiddles[i - 1] * wlen % p);
        }

        if (len / 2 >= 4) {
            butterfly_layer_simd(a, n, len, p, twiddles);
        } else {
            for (int i = 0; i < n; i += len) {
                int w_now = 1;
...
void poly_multiply(int *a, int *b, int *ab, int n, int p) {
    ...
    int *ta = (int*)aligned_alloc(64, m * sizeof(int));
    int *tb = (int*)aligned_alloc(64, m * sizeof(int));
    if (!ta || !tb) {
        fprintf(stderr, "内存分配失败\n");
        free(ta);
        free(tb);
